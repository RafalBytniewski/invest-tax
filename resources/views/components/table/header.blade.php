...
<th {{ $attributes->except(['wire:click', 'wire:target', 'wire:loading.attr', 'title', 'aria-label'])->merge(['class' => 'text-left px-6 py-3 bg-gray-50 dark:bg-gray-800']) }}>
    @unless($sortable)
    ...
    @else
        <button
    {{ $attributes->only(['wire:click', 'wire:target', 'wire:loading.attr', 'title', 'aria-label']) }}
