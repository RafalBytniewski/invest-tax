            ...
            <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    ...
                    <p class="mt-2 max-w-3xl text-sm text-gray-500 dark:text-zinc-400">
                        Unified filters, table spacing and controls consistent with the rest of the app.
                    </p>
                </div>

                <button wire:click="$dispatch('openNewTransactionModal')"
                    class="inline-flex h-11 items-center justify-center rounded-xl bg-blue-600 px-5 text-sm font-semibold text-white transition hover:bg-blue-500">
                    New Transaction
                </button>
            </div>
            ...
            <div class="grid gap-3 xl:grid-cols-[minmax(0,1fr)_auto]">
                <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_180px_180px]">
                    <div class="space-y-2">
                        ...
                        <input type="text" wire:model.live.debounce.500ms="search" placeholder="Find by asset, wallet or broker"
                            class="h-11 w-full rounded-xl border border-gray-200 bg-white px-4 text-sm text-gray-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-white" />
                    </div>
                ...
                <div class="space-y-2 xl:w-32">
                    ...
                    <select wire:model.live="perPage"
                        class="h-11 w-full rounded-xl border border-gray-200 bg-white px-4 text-sm text-gray-900 outline-none transition hover:bg-gray-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-white dark:hover:bg-zinc-900">
                    ...
                    <x-slot name="body">
                        @forelse ($transactions as $transaction)
                            <x-table.row wire.loading.class.delay="opacity-50">
                                ...
                                <x-table.cell class="px-4 py-3 text-right">
                                    {{ number_format($transaction->price_per_unit, 2, ',', ' ') }}
                                    <span class="text-xs text-gray-500 dark:text-zinc-400">
                                        {{ $transaction->asset->exchange->currency ?? $transaction->currency }}
                                    </span>
                                </x-table.cell>
                                <x-table.cell class="px-4 py-3 text-right">
                                ...
                                <x-table.cell class="w-32 text-center">
                                    <div class="flex justify-center space-x-1">
                                        <button wire:click="$dispatchTo('transaction-form-modal', 'openTransactionFormModal', { transactionId: {{ $transaction->id }} })"
                                        ...
                                        <button wire:click="deleteTransaction({{ $transaction->id }})" wire:confirm="Delete this transaction?"
                                            class="rounded-lg p-1 transition-colors hover:bg-red-100 dark:hover:bg-red-900/30">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px" fill="#dc2626">
                            ...
                            <x-table.row>
                                <x-table.cell colspan="9">
                                    <div class="flex items-center justify-center py-10">
                                        <span class="text-base font-medium text-gray-500 dark:text-zinc-400">
